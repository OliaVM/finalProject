...
if (isset($_SESSION['login']) && isset($_SESSION['password'])) {
	if (isset($_POST['image_go'])) { //если пользователь нажал на кнопку поставить лайк
			$sql9 = 'SELECT * FROM likes WHERE user_id = :user_id and like_id = :like_id'; //проверяем не голосовал ли уже пользователь за эту новость
			...
			try {
				...
				if (empty($rowLike['like_count'])) {
						...
						$sql = 'UPDATE articles  SET  like_number =:like_number WHERE id='.$id.''; //Записываем результат голосования
